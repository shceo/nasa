import 'dart:io';
import 'dart:convert';
import 'package:http/http.dart' as http; // Подключаем http для запросов к API NASA
...
void main() async {
  ...
  var apiHandler = (Request request) async {
    if (request.url.path == 'api/nasa') {
      ...
      var nasaApiUrl = 'https://api.nasa.gov/planetary/apod?api_key=YOUR_NASA_API_KEY';

      // Отправляем запрос к NASA API
      ...
      if (response.statusCode == 200) {
        ...
        return Response.internalServerError(body: 'Error fetching data from NASA API');
      }
    } else {
  ...
  // Запуск сервера
  var server = await io.serve(pipeline.addHandler(cascade.handler), 'localhost', 8080);
